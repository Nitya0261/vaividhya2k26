...
function Paid() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // HARDCODED PASSWORD (Replace "admin123" with your desired password)
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      const response = await fetch(
        `https://vaividhya-backend.onrender.com/api/registrations/${id}/mark-paid`,
        {
          method: "PATCH",
  ...
  const handleMarkUnpaid = async () => {
    ...
    try {
      const response = await fetch(
        `https://vaividhya-backend.onrender.com/api/registrations/${id}/mark-unpaid`, 
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        }
      ...
      <div className="vh-100 d-flex justify-content-center align-items-center bg-dark text-white">
        <div className="card bg-secondary bg-opacity-25 p-4 shadow-lg border-secondary" style={{maxWidth: '400px', width: '100%'}}>
          ...
          <form onSubmit={handleLogin}>
            <div className="mb-3">
              <label className="form-label text-white-50 small text-uppercase fw-bold">Enter Password</label>
              <input 
                type="password" 
