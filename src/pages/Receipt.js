...
function Receipt() {
  ...
  useEffect(() => {
    ...
    async function fetchReceipt() {
      try {
        const response = await fetch(`https://vaividhya-backend.onrender.com/api/registrations/${userId}`);
            ...
            <div className="card-body p-4 p-md-5">
              ...
              <div className="row mb-4">
                ...
                <div className="col-md-4 text-center border-start-md mt-4 mt-md-0 d-flex flex-column justify-content-center align-items-center">
                  <img 
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=http://vaividhya2k26.in/paid?userId=${_id}`} 
                    alt="Entry QR" 
                  ...
                  {payment_status === "PAID" ? (
                    <span className="badge rounded-pill bg-success px-4 py-2 fs-6 print-badge">PAID</span>
                  ) : (
                    <span className="badge rounded-pill bg-danger px-4 py-2 fs-6 print-badge">PENDING</span>
                  )}
                  <small className="text-muted mt-2 d-block">Scan at Entry</small>
                </div>
              </div>
